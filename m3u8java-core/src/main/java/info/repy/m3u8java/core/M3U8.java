...
public class M3U8 extends AbstractExecuter {
    ...
    private static class TSDownload {
        private static final int RETRY = 10; // mpegtsが188でワンパケットのため188
        ...
        public TSDownload(URL url, KeyType type, byte[] key, byte[] iv, String referer) {
            this.url = url;
            this.type = type;
            this.key = key.clone();
            this.iv = iv.clone();
                    ...
                    try {
                        ...
                        for (int retry = 1; retry <= RETRY; retry++) {
                            try {
                                ...
                                try (CipherInputStream cipherInputStream = new CipherInputStream(new BufferedInputStream(Util.http(TSDownload.this.getUrl(), 10*60, TSDownload.this.referer)), cipher)) {
                                    byte[] bytes = cipherInputStream.readAllBytes();
                                    if (bytes.length % TS_PACKET_SIZE != 0) {
                                        throw new IOException("MPEG2 TSでないファイル");
                                    }
    ...
    protected void run(AsyncListener listener) {
        ...
        try {
            ...
            try (BufferedReader br = new BufferedReader(new InputStreamReader(Util.http(m3u8Url, 60, referer)))) {
                String line;
            ...
            try (BufferedReader br = new BufferedReader(new InputStreamReader(Util.http(mediaURL, 60, referer)))) {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (property == null) {
                        ...
                        try (InputStream ks = Util.http(keyUrl, 60, referer)) {
                        ...
                        list.add(new TSDownload(tsUrl, type, key, iv, this.referer));
    ...
    private static final InputStream http(URL url, int timeout, String referer) throws IOException, URISyntaxException, InterruptedException {
        String origin = null;
        if (referer != null) {
            try {
